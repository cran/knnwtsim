...
test_that("NNreg returns average of correct k nearest neighbors", {
  ...
  # if v.test represents similarity the 1st, 2nd,
  # and 5th observations in y.test are neighbors
...
test_that("knn.forecast throws errors for bad y.in and f.index.in arguments", {
  ...
  expect_error(knn.forecast(
    Sim.Mat.in = Sim.Mat.test, f.index.in = f.index.ch,
    k.in = k.test, y.in = y.test
  ))
  ...
  expect_error(knn.forecast(
    Sim.Mat.in = Sim.Mat.test, f.index.in = f.index.mat,
    k.in = k.test, y.in = y.test
  ))
})
...
test_that("knn.forecast throws errors for bad Sim.Mat.in arguments", {
  ...
  expect_error(knn.forecast(
    Sim.Mat.in = ch.mat, f.index.in = f.index.test,
    k.in = k.test, y.in = y.test
  ))
  ...
  expect_error(knn.forecast(
    Sim.Mat.in = Sim.vec, f.index.in = f.index.test,
    k.in = k.test, y.in = y.test
  ))

  # test for non-symmetric Sim.Mat.in
  expect_error(knn.forecast(
    Sim.Mat.in = Sim.Mat.nosymmetric, f.index.in = f.index.test,
    k.in = k.test, y.in = y.test
  ))
})
...
test_that("knn.forecast throws warnings and forecasts correctly
  ...
  expect_equal(suppressWarnings(knn.forecast(
    Sim.Mat.in = Sim.Mat.test, f.index.in = f.index.test,
    k.in = k.test, y.in = y.test
  )), expected.forecast)
})
...
test_that("knn.forecast throws errors when there are less eligible
          neighbors than k.in", {
  Sim.Mat.test <- matrix(c(
    1, .2, .3, .4, .5,
    .2, 1, .4, .3, .2,
